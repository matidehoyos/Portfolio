...
const Contact = () => {
    ...
    return(
        <div className={style.container} id="contact" style={{background: theme === 'light' ? "#eee" : ''}}>
                <p className={style.contactMe} data-aos="fade-right" data-aos-duration="800" id='about' style={{color: theme === 'dark' ? '#aaa' : '#111', fontWeight: theme === 'dark' ? '400' : '600', borderBottom: theme === 'dark' ? '' : '2px solid #111' }}>
                    {t('contact.title')}
                </p>
                <div className={`${style.box} ${isLightMobile ? style.boxLight : style.box}`} data-aos="fade-up" data-aos-duration="800" style={{background: theme === 'light' ? '#444' : '', border: theme === 'light' ? '1px solid #00000030' : ''}}>
                ...
                <div className={`${style.caja} ${isLightMobile ? style.cajaLight : style.caja}`}>
                    ...
                    <div className={`${style.redes} ${isLightMobile ? style.redesLight : style.redes}`}>
                        <nav>
                            <ul>
                                <li><a href='mailto:mati.dehoyosmdp@gmail.com' target="_blank" className={isLightMobile ? style.mail : ''}  style={{background: theme === 'light' ? '#fff' : '', border: theme === 'light' ? '1px solid #ccc' : '', color: theme === 'light' ? '#111' : '', fontWeight: theme === 'light' ? '400' : ''}}><FaEnvelope className={style.icon}/>mati.dehoyosmdp@gmail.com</a></li>
                                <li><a href="https://api.whatsapp.com/send?phone=542236901801" target="_blank" className={isLightMobile ? style.red : ''} style={{background: theme === 'light' ? '#ffffff80' : '', border: theme === 'light' ? '1px solid #fff' : '', color: theme === 'light' ? '#000' : ''}}><FaWhatsapp className={style.icon}/>WhatsApp</a></li>
